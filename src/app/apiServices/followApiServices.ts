...
class FollowApiService {
  ...
  public async getMemberFollowers(data: FollowSearchObj): Promise<Follower[]> {
    try {
      ...
      console.log(`ERROR ::: getFollowers", ${err.message}`);
      throw err;
    }
  }
  public async getMemberFollowings(data: FollowSearchObj): Promise<Following[]> {
    try {
      ...
      console.log(`ERROR ::: getFollowings", ${err.message}`);
      throw err;
    }
  }
  ...
  public async subscribe(mb_id: string): Promise<boolean> {
    try {
      ...
      return result.data.data === "subscribe";
    } catch (err: any) {
  ...
  public async unsubscriber(mb_id: string): Promise<boolean> {
    try {
      ...
      return result.data.data === "unsubscriber";
    } catch (err: any) {
      console.log(`ERROR ::: unsubscriber", ${err.message}`);
      throw err;
    }
  }
}
